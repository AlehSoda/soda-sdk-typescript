...
export async function decryptRSAMessage(privateKey, encryptedData) {
    return await crypto.subtle.decrypt({
        name: "RSA-OAEP"
    }, privateKey, encryptedData);
}
export async function importRSAPrivateKey(privateKeyData) {
...
export async function prepareIT(plaintext, walletAddress, walletPrivateKey, userKey, contractAddress, functionSelector) {
    ...
    const message = solidityPackedKeccak256(["address", "address", "bytes4", "uint256"], [walletAddress, contractAddress, functionSelector, BigInt("0x" + ct)]);
